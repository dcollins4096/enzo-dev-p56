			       ...
			       HierarchyEntry &TopGrid, TopGridData &MetaData)
{
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) {
    ...
    if( sscanf(line, "TurbulenceSimulationInitialTemperatureKelvin = %"FSYM,
                  &TurbulenceSimulationInitialTemperature) ){
            ret += 1;
            TurbulenceSimulationInitialTemperature /= TemperatureUnits;
            }
    ...
    if( sscanf(line, "TurbulenceSimulationInitialMagneticFieldGauss = %"PSYM" %"PSYM" %"PSYM,
		  TurbulenceSimulationInitialMagneticField,
		  TurbulenceSimulationInitialMagneticField+1,
	       TurbulenceSimulationInitialMagneticField+2) > 0){
      for( i=0;i<3;i++)
          TurbulenceSimulationInitialMagneticField[i] /= MagneticUnits;
      ret++;
      InitialMagneticFieldDefined = TRUE;
    }
  ...
  */
  ...
  if ( (UseDrivingField || RandomForcing) && RandomForcingEdot < 0.0) {
  ...
  if (RandomForcing || UseDrivingField)
    if (MyProcessorNumber == ROOT_PROCESSOR)
      fprintf(stderr,"RandomForcingEdot: %"GSYM" RandomForcingMachNumber %"GSYM"\n", RandomForcingEdot, RandomForcingMachNumber);
