                          ...
                          int   TotalRefinement)
{
  ...
  */
  ...
  if (RandomForcing == TRUE && ReadData == TRUE && ! UseDrivingField)
  ...
  /* OR: copy random forcing fields from initial velocities. */
    if( ReadData ){ 
      ...
      if (ReadData == TRUE && RandomForcing == TRUE && !UseDrivingField){
        for (dim = 0; dim < GridRank; dim++)
          for (i = 0; i < size; i++)
            RandomForcingField[dim][i] = BaryonField[vel+dim][i];
      }
      if (ReadData == TRUE && UseDrivingField){
        for (dim = 0; dim < GridRank; dim++)
          for (i = 0; i < size; i++)
              BaryonField[idrivex+dim][i] = BaryonField[vel+dim][i];
      }
  }
  ...
  fprintf(stderr,"CLOWN bf %0.2e %0.2e\n", BaryonField[0][8*8*8], BaryonField[TENum][8*8*8]);
  }//ReadData
